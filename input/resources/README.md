...
| `v2027.*` | 2026-05-01 *(vorläufig: am SU-TermServ gemessen 2026-09-03; Wiki-Zeile/TF-KDS-Bestätigung ausstehend)* | `http://snomed.info/sct/900000000000207008/version/20260501` |

The shipped file carries the **`v2027.*` pin (vorläufig, s. Tabelle)**, matching the 2026
...
1. **Replace `biobank` inside the file** — it appears in the resource
   `id` (`mii-param-biobank-manifest`). A `{{…}}` left in a FHIR `id` is
   ...
   `input/translations/de/ImplementationGuide-mii-ig-biobank-de-v2026.po`, which
